...
Guest 应用的 0.4 版线缆格式**仍为上述 10 色 / 60 字节**：Host 只映射现有的十个系统色，不会把系统的次要色、第三色或容器色直接传给 Guest。要让 Guest 独立读取这些角色，需要后续设计带版本协商的新 ABI；不可直接扩大当前快照。预设为静态配色，不包含 Android 的壁纸取色或动态 HCT 生成功能。
