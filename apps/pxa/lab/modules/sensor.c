#define PXA_LAB_MODULE_PREFIX pxa_lab_sensor_
...
static int is_temperature_semantic(const pxa_sensor_descriptor_t *descriptor) {
    static const char semantic[] = "ambient.temperature";
...
static int render(void) {
    ...
    if (subscription_handle != 0 && has_sample) {
        size_t value_length = pxa_ui_demo_format_u32(value,
            (uint32_t)(temperature_milli_c < 0 ? -temperature_milli_c : temperature_milli_c));
        if (temperature_milli_c < 0 && value_length + 1 < sizeof(value)) {
    ...
    page.slider_value = has_sample ? (uint8_t)((uint32_t)(temperature_milli_c < 0 ? 0 : temperature_milli_c) % 101u) : 0;
...
static int acquire_permission(void) {
    ...
    static const uint8_t scope[] = "ambient.temperature";
    waiting = 1;
    failed = 0;
...
int32_t pxa_app_on_event(const uint8_t *event, uint32_t length) {
    ...
    if (parsed.service == PXA_SERVICE_SENSOR && parsed.opcode == PXA_SENSOR_LIST &&
        ...
        while (pxa_sensor_descriptor_next(&result, &offset, &descriptor)) {
            if (is_temperature_semantic(&descriptor)) {
    ...
    if (parsed.service == PXA_SERVICE_SENSOR && parsed.opcode == PXA_SENSOR_SAMPLE) {
        ...
        temperature_milli_c = (int32_t)pxa_read_u32(sample.values);
        has_sample = 1;
        return render() ? PXA_EVENT_HANDLED : PXA_STATUS_INTERNAL;
    }
