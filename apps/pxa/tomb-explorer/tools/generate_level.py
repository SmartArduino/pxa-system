#!/usr/bin/env python3
"""Compile tools/level.json into level_data.c for the PXA tomb-explorer port.

Ported from micropixel guest/apps/tomb-explorer/tools/generate_level.py; the
mesh building code is unchanged, only the checked-in output is plain C. Rooms
are sector grids. From the per-corner floor and ceiling heights the generator
emits floor, ceiling, wall, step and ceiling-drop quads with baked vertex
light, the portal quads between rooms and the collision sector table. Use
--check to verify the checked-in C matches the description.

Coordinates: x east, y up, z south (north is -z). Faces are listed
counter-clockwise as seen from inside the room.
"""
import argparse
import json
import math
...
TEXELS_PER_UNIT = 64
MAX_WALL_PIECE = 2.0     # units of height per wall quad, keeps texel spans under 256
WATER_DEPTH = 0.3
MAX_VERTICES = 1024      # tomb mesh vertex limit
...
class MeshBuilder:
    ...
    def face(self, corners, uvs, texture, flags=0):
        ...
        if max(us) - min(us) >= 256 or max(vs) - min(vs) >= 256:
            raise ValueError(f'{self.room.name}: face texel span exceeds 255')
        # Shift into 0..65535 wire range; the renderer rebases on 256 anyway.
        shift_u = math.floor(min(us) / 256) * 256
        shift_v = math.floor(min(vs) / 256) * 256
        self.faces.append({
            'vertex': [self.vertex(*c) for c in corners],
            'u': [int(round(u - shift_u)) for u in us],
            'v': [int(round(v - shift_v)) for v in vs],
    ...
    def build(self):
        ...
        if len(self.vertices) > MAX_VERTICES:
            raise ValueError(f'{self.room.name}: {len(self.vertices)} vertices exceed the renderer limit {MAX_VERTICES}')
...
def texture_ident(name):
    """kTexMossyWall -> TEX_MOSSY_WALL (the names in textures.h)."""
    if not name.startswith('k'):
        raise ValueError(f'unknown texture id {name!r}')
...
def render(level):
    ...
    for room in level.rooms:
        ...
        counts.append((len(builder.vertices), len(builder.faces), len(portals)))
        out.append(f'static const tomb_vertex_t k{name}Vertices[{len(builder.vertices)}] = {{')
        for x, y, z in builder.vertices:
            out.append(f'    {{{c_float(x)}, {c_float(y)}, {c_float(z)}}},')
        out.append('};')
        out.append(f'static const tomb_face_t k{name}Faces[{len(builder.faces)}] = {{')
        for face in builder.faces:
            v = ', '.join(f'{i}u' for i in face['vertex'])
            u = ', '.join(f'{i}u' for i in face['u'])
            vv = ', '.join(f'{i}u' for i in face['v'])
            light = ', '.join(f'{i}u' for i in face['light'])
            out.append(f'    {{{{{v}}}, {{{u}}}, {{{vv}}}, {{{light}}}, '
                       f'{texture_ident(face["texture"])}, {face["flags"]}u}},')
        out.append('};')
        out.append(f'static const tomb_sector_t k{name}Sectors[{len(room.sectors)}] = {{')
        for sector in room.sectors:
            floor = ', '.join(c_float(h) for h in sector.floor)
            ceiling = ', '.join(c_float(h) for h in sector.ceiling)
            out.append(f'    {{{{{floor}}}, {{{ceiling}}}, {"1" if sector.solid else "0"}}},')
        out.append('};')
        if portals:
            out.append(f'static const tomb_portal_t k{name}Portals[{len(portals)}] = {{')
            for portal in portals:
                corners = ', '.join(f'{{{c_float(x)}, {c_float(y)}, {c_float(z)}}}' for x, y, z in portal['corners'])
    ...
    for index, room in enumerate(level.rooms):
        name = ident(room.name)
        vertex_count, face_count, portal_count = counts[index]
        portals = f'k{name}Portals' if portal_count else '0'
        out.append(f'    {{{c_float(room.origin_x)}, {c_float(room.origin_z)}, {room.width}, {room.depth}, '
                   f'{room.ambient}, {portal_count}, {len(room.sectors)}, k{name}Sectors, '
                   f'{vertex_count}, k{name}Vertices, {face_count}, k{name}Faces, {portals}}},')
    out.append('};')
    ...
    out += [
        '',
        f'static const tomb_level_t kLevel = {{kRooms, {len(level.rooms)}, '
        f'{{{c_float(start["x"])}, {c_float(start_y)}, {c_float(start["z"])}}}, '
        f'{c_float(start["yaw"])}, {level.room_index(start["room"])}}};',
        '/* clang-format on */',
        '',
...
def main():
    ...
    parser.add_argument('--check', action='store_true', help='verify level_data.c is up to date')
    args = parser.parse_args()
    level = Level(json.loads((APP / 'tools/level.json').read_text()))
    text, stats = render(level)
    output = APP / 'level_data.c'
    if args.check:
        if not output.exists() or output.read_text() != text:
            raise SystemExit('stale: level_data.c')
        print('tomb-explorer level up to date')
        return
    output.write_text(text)
    for name, vertices, faces, portals in stats:
        print(f'{name}: {vertices} vertices, {faces} faces, {portals} portals')
    print(f'wrote {output.relative_to(APP)}')
