...
static uint8_t current_wave(void) {
    return (uint8_t)(1u + score / WAVE_SCORE_STEP);
}

static void reset_game(void) {
    score = 0;
    combo = 0;
    lives = (uint8_t)(3u + configured_hull_level);
    energy = 0;
    star_scroll = 0;
    fire_counter = 0;
    spawn_counter = 0;
    ...
    game_over = 0;
    last_tick_us = 0;
    player_x = (int16_t)(PLAYER_LANE_X - configured_ship_model * 3u);
    player_y = 132;
...
static void start_boss(uint8_t wave) {
    ...
    boss.y = 95;
    boss.vy = 1;
    boss.max_health = (uint8_t)(30u + wave * 8u);
...
static void maybe_start_boss(void) {
    const uint8_t wave = current_wave();
    if (!boss.active && wave >= BOSS_WAVE_INTERVAL &&
        wave % BOSS_WAVE_INTERVAL == 0 && boss_wave != wave)
        start_boss(wave);
}
...
static void draw_player(pxa_canvas_frame_t* frame) {
    ...
    if (invulnerable_ticks == 0 || (invulnerable_ticks / 3u) % 2u == 0)
        pxa_canvas_image(frame, player_x, (int16_t)(player_y - 2), PLAYER_PLANE_W,
                         PLAYER_PLANE_H, 255, 0, player_asset, sizeof(player_asset) - 1);
}
...
static void draw_boss(pxa_canvas_frame_t* frame) {
    ...
    pxa_canvas_image(frame, boss.x, boss.y, BOSS_WIDTH, BOSS_HEIGHT, 255,
                     PXA_UI_IMAGE_FIT_STRETCH, boss_asset, sizeof(boss_asset) - 1);
    if (enraged)
...
static int render(void) {
    ...
    pxa_canvas_rect(&frame, 0, 0, GAME_WIDTH, GAME_HEIGHT, 0x061321, 0);
    ...
    const char *wave_label = PXA_ARCADE_MSG(PXA_MSG_SHOOTER_HUD_WAVE);
    const char *chain_label = PXA_ARCADE_MSG(PXA_MSG_SHOOTER_HUD_CHAIN);
    ...
    pxa_canvas_text_box(&frame, 129, 6, 22, 24, 0x95C5E8, PXA_CANVAS_ALIGN_RIGHT,
                        ...
                        pxa_arcade_text_size(wave_label));
    ...
    pxa_canvas_text_box(&frame, 177, 6, 22, 24, 0x95C5E8, PXA_CANVAS_ALIGN_RIGHT,
                        ...
                        pxa_arcade_text_size(chain_label));
    ...
    if (boss.active) {
        static const char boss_label[] = "BOSS";
        pxa_canvas_text_box(&frame, 34, 40, 60, 16, 0xFF9FB0, PXA_CANVAS_ALIGN_CENTER,
                            PXA_CANVAS_TEXT_ALIGN_MIDDLE, boss_label, sizeof(boss_label) - 1);
    ...
    if (overdrive_ticks != 0) {
        ...
        const char *wave_notice = PXA_ARCADE_MSG(PXA_MSG_SHOOTER_WAVE_INCOMING);
        ...
        pxa_canvas_text_box(&frame, 134, 44, 56, 20, 0xD9F2FF, PXA_CANVAS_ALIGN_LEFT,
                            ...
                            pxa_arcade_text_size(wave_notice));
    }
    if (score == 0 && !game_over && wave_notice_ticks == 0) {
        const char *hint = PXA_ARCADE_MSG(PXA_MSG_SHOOTER_HINT_DODGE);
        pxa_canvas_text_box(&frame, 90, 217, 130, 18, 0x8EB6D4, PXA_CANVAS_ALIGN_CENTER,
                            PXA_CANVAS_TEXT_ALIGN_MIDDLE, hint,
                            pxa_arcade_text_size(hint));
    }
    if (game_over) {
        const char *over = PXA_ARCADE_MSG(PXA_MSG_SHOOTER_RESULT_LOST);
        ...
        pxa_canvas_text(&frame, 50, 105, 196, 0xFFFFFF, PXA_CANVAS_ALIGN_CENTER,
                        over, pxa_arcade_text_size(over));
    }
