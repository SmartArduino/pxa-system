...
"${PYTHON:-python3}" \
  "$project_dir/pxa-system/tools/package/test_ui_v2_vectors.py" \
  "$project_dir/pxa-system/spec/draft-0.2/golden/ui-v2-vectors.json"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_net_test.c" \
  -o "$work_dir/pxa_net_test"
"$work_dir/pxa_net_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_device_test.c" \
  -o "$work_dir/pxa_device_test"
"$work_dir/pxa_device_test"

"${PYTHON:-python3}" - "$project_dir" <<'PYTHON'
...
tool_path = project_dir / "pxa-system/tools/package/build_package_manifest.py"
...
invalid = []
for assets_dir in (project_dir / "apps/pxa").glob("*/assets"):
...
for relative, expected_size in expected_pngs.items():
    data = (project_dir / relative).read_bytes()
    if data[:8] != b"\x89PNG\r\n\x1a\n" or len(data) < 24:
...
garden_source = (
    project_dir / "apps/pxa/garden-guard/main.c"
...
arcade_source = (project_dir / "apps/pxa/arcade/main.c").read_text(encoding="utf-8")
...
"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_canvas_test.c" \
  -o "$work_dir/pxa_canvas_test"
"$work_dir/pxa_canvas_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_ui_builder_test.c" \
  -o "$work_dir/pxa_ui_builder_test"
"$work_dir/pxa_ui_builder_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_fs_test.c" \
  -o "$work_dir/pxa_fs_test"
"$work_dir/pxa_fs_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_storage_test.c" \
  -o "$work_dir/pxa_storage_test"
"$work_dir/pxa_storage_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_ipc_test.c" \
  -o "$work_dir/pxa_ipc_test"
"$work_dir/pxa_ipc_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_lease_test.c" \
  -o "$work_dir/pxa_lease_test"
"$work_dir/pxa_lease_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_sensor_test.c" \
  -o "$work_dir/pxa_sensor_test"
"$work_dir/pxa_sensor_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_permission_test.c" \
  -o "$work_dir/pxa_permission_test"
"$work_dir/pxa_permission_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_work_test.c" \
  -o "$work_dir/pxa_work_test"
"$work_dir/pxa_work_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_audio_test.c" \
  -o "$work_dir/pxa_audio_test"
"$work_dir/pxa_audio_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_surface_test.c" \
  -o "$work_dir/pxa_surface_test"
"$work_dir/pxa_surface_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_game_sfx_test.c" \
...
for app_source in \
  ...
  "$cc_bin" "${flags[@]}" "${app_flags[@]}" \
    "$project_dir/components/pxa/tests/pxa_canvas_smoke_test.c" \
...
for app_source in \
  ...
  "$cc_bin" "${flags[@]}" -DPXA_ARCADE_STANDALONE_TEST \
    "$project_dir/components/pxa/tests/pxa_pointer_frame_pacing_test.c" \
    "$project_dir/$app_source" \
...
"$cc_bin" "${flags[@]}" \
  ...
  "$project_dir/components/pxa/tests/pxa_tetris_controls_test.c" \
  "$project_dir/apps/pxa/arcade/modules/tetris.c" \
  -o "$work_dir/pxa_tetris_controls_test"
"$work_dir/pxa_tetris_controls_test"

for app_id in arcade weather lab; do
  "$cc_bin" "${flags[@]}" -fsyntax-only \
    "$project_dir/apps/pxa/$app_id/main.c"
done
for lab_module in "$project_dir"/apps/pxa/lab/modules/*.c; do
  "$cc_bin" "${flags[@]}" -fsyntax-only "$lab_module"
done
"$cc_bin" "${flags[@]}" -fsyntax-only \
"$project_dir/apps/pxa/lab/responder.c"
"$cc_bin" "${flags[@]}" -fsyntax-only \
  "$project_dir/apps/pxa/lab/worker.c"

"$cc_bin" "${flags[@]}" -DPXA_ARCADE_STANDALONE_TEST \
  "$project_dir/components/pxa/tests/pxa_jump_jump_test.c" \
  -o "$work_dir/pxa_jump_jump_test"
"$work_dir/pxa_jump_jump_test"

"$cc_bin" "${flags[@]}" \
  "$project_dir/components/pxa/tests/pxa_garden_guard_test.c" \
