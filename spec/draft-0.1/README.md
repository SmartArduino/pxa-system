...
UI 0.1 files in this directory are retained only as the historical Draft 0.1
snapshot. Current Hosts and Packages use the incompatible UI service 0.3 in
`../draft-0.2`; no UI 0.1 translation or compatibility implementation exists.
...
- [ui-canvas.md](ui-canvas.md) defines atomic retained UI transactions and the
  backend-neutral Canvas display-list node.
...
- [pxa-core.yaml](pxa-core.yaml) is the machine-readable source of truth for
  Core symbols and numeric assignments.
- [pxa-container.yaml](pxa-container.yaml) is the machine-readable source of
  truth for `.pxa` container layouts and numeric assignments.
- [pxa-window.yaml](pxa-window.yaml), [pxa-ui.yaml](pxa-ui.yaml),
  [pxa-clock.yaml](pxa-clock.yaml), [pxa-fs.yaml](pxa-fs.yaml),
  [pxa-storage.yaml](pxa-storage.yaml), [pxa-ipc.yaml](pxa-ipc.yaml),
  [pxa-sensor.yaml](pxa-sensor.yaml), [pxa-net.yaml](pxa-net.yaml),
  [pxa-audio.yaml](pxa-audio.yaml), [pxa-permission.yaml](pxa-permission.yaml),
  [pxa-device.yaml](pxa-device.yaml), and [pxa-work.yaml](pxa-work.yaml) define
  published Draft service assignments.
- `golden/core-vectors.json` contains reproducible wire examples.
- `tools/check_spec.rb` validates the specification and golden vectors without
  third-party dependencies.

## Service namespace

Draft 0.1 reserves the following top-level services. Reserving a service ID is
